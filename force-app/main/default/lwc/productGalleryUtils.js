...
export function getImageSizes(el) {
    // Avoid optional chaining and globalThis to support strict LWC transpilation during deploy
    const styles = el && typeof window !== 'undefined' && window.getComputedStyle ? window.getComputedStyle(el) : null;
    const mobile = Number(styles ? styles.getPropertyValue('--com-c-image-width-mobile') : null);
    const tablet = Number(styles ? styles.getPropertyValue('--com-c-image-width-tablet') : null);
    const desktop = Number(styles ? styles.getPropertyValue('--com-c-image-width-desktop') : null);
    return {
